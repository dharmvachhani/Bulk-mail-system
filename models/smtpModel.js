...
const create = function (data, callback) {
  con.query("INSERT INTO smtp SET ?", data, function (err, result) {
    if (err) {
...
const get = function (callback) {
  con.query("SELECT * FROM smtp WHERE is_deleted = 0", function (err, result) {
    if (err) {
      // console.log("error: ", err);
      callback(err, null);
      return;
...
const getById = function (id, callback) {
  con.query("SELECT * FROM smtp WHERE id = ?", id, function (err, result) {
    if (err) {
      // console.log("error: ", err);
      callback(err, null);
      return;
...
const deleteById = function (id, callback) {
  con.query("UPDATE smtp SET is_deleted = 1 WHERE id = ?", id, function (err, result) {
    if (err) {
      // console.log("error: ", err);
      callback(err, null);
      return;
